...
void LocalMapping::MapPointCulling()
{
    ...
    const int cnThObs = nThObs; // 被观测到的次数 阈值

    int borrar = mlpRecentAddedMapPoints.size();

    /// 遍历新引入的地图点，剔除质量不好的地图点
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        {
            // 已经过了两个关键帧 并且 被观测到的次数很少
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        else if(((int)nCurrentKFid-(int)pMP->mnFirstKFid)>=3) // 已经过了三个关键帧
            lit = mlpRecentAddedMapPoints.erase(lit);
        else
        {
            lit++;
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        else
        {
            ...
            if(ratioBaselineDepth<0.01)
                continue; // 基线与深度的比例特别小，说明两个关键帧相距太近
        }
        ...
        {
            const int &idx1 = vMatchedIndices[ikp].first;  // 相机1
            const int &idx2 = vMatchedIndices[ikp].second; // 相机2
            ...
            {
                continue; //No stereo and very low parallax 单目视差角较小时 生成不了三维点
            }
            ...
            const float ratioDist = dist2/dist1; // 深度比值
...
/// 检查并融合 当前关键帧 与 相邻帧（一级二级相邻帧）重复的地图点
void LocalMapping::SearchInNeighbors()
{
    // Retrieve neighbor keyframes
    /// 寻找当前关键帧的 一级相邻帧 以及 二级相邻帧
    int nn = 10;
    if(mbMonocular)
        nn=20; // 10帧 一级相邻帧
    const vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);
    vector<KeyFrame*> vpTargetKFs; // 相邻帧列表
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        ...
        pKFi->mnFuseTargetForKF = mpCurrentKeyFrame->mnId; // 加入集合，并标记已加入
    }
    ...
    ORBmatcher matcher;
    vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches(); // 当前帧的地图点
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;

        matcher.Fuse(pKFi,vpMapPointMatches); /// 1.当前帧的地图点 与 相邻帧 融合
    ...
    for(vector<KeyFrame*>::iterator vitKF=vpTargetKFs.begin(), vendKF=vpTargetKFs.end(); vitKF!=vendKF; vitKF++)
    {
        ...
        for(vector<MapPoint*>::iterator vitMP=vpMapPointsKFi.begin(), vendMP=vpMapPointsKFi.end(); vitMP!=vendMP; vitMP++)
        {
            ...
            if(pMP->isBad() || pMP->mnFuseCandidateForKF == mpCurrentKeyFrame->mnId)
                continue;
            pMP->mnFuseCandidateForKF = mpCurrentKeyFrame->mnId; // 加入集合，并标记已加入
            vpFuseCandidates.push_back(pMP); // 相邻帧的地图点
        }
    }

    matcher.Fuse(mpCurrentKeyFrame,vpFuseCandidates); /// 2.相邻帧的地图点 与 当前帧 融合
...
void LocalMapping::KeyFrameCulling()
{
    ...
    {
        count++;
        KeyFrame* pKF = *vit; // 共视帧

        if((pKF->mnId==pKF->GetMap()->GetInitKFid()) || pKF->isBad())
            continue;
        const vector<MapPoint*> vpMapPoints = pKF->GetMapPointMatches(); // 共视帧的地图点

        int nObs = 3;
        const int thObs=nObs;
        int nRedundantObservations=0; // 多余的地图点个数（被其他关键帧（3帧以上） 观测到的地图点，为多余地图点）
        int nMPs=0;
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    if(pMP->Observations()>thObs)
                    {
                        ...
                        int nObs=0; // 观测到 该地图点 的关键帧个数
                        for(map<KeyFrame*, tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            KeyFrame* pKFi = mit->first;
                            if(pKFi==pKF) // 不包括 当前共视帧
                                continue;
                        ...
                        if(nObs>thObs) // 观测到 该地图点 的关键帧个数 大于3
                        {
                            nRedundantObservations++; // 该地图点是多余的
                        }
                    }
                }
            }
        ...
        if(nRedundantObservations>redundant_th*nMPs)
        {
            ...
            {
                pKF->SetBadFlag(); // 当前共视帧，有90%以上的地图点是多余，则该共视帧也是多余的
            }
        }
