...
/// 通过前后两帧已经匹配好的特征点，来进行初始化（对极几何、三角测量）
...
/// 使用 直接线性法DLT，计算 单应矩阵 H
cv::Mat Initializer::ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2)
{
    const int N = vP1.size(); // 点对的数量，应该为8

    /// 8对点，计算单应矩阵（其实四对点就够了）
    cv::Mat A(2*N,9,CV_32F); // 16X9（8X9）
...
/// 通过基础矩阵F 计算R t（有四组）; 再通过三角化成功的特征点数量 来筛选R t
                            ...
                            cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    float parallax1,parallax2, parallax3, parallax4; /// 第50大的视差，用来判断是否有50组匹配点的视差足够大

    // 分别计算四组R t的 三角化成功的特征点数量
    int nGood1 = CheckRT(R1,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D1, 4.0*mSigma2, vbTriangulated1, parallax1);
    int nGood2 = CheckRT(R2,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D2, 4.0*mSigma2, vbTriangulated2, parallax2);
    int nGood3 = CheckRT(R1,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D3, 4.0*mSigma2, vbTriangulated3, parallax3);
    int nGood4 = CheckRT(R2,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D4, 4.0*mSigma2, vbTriangulated4, parallax4);

    // 四组结果中，理论上有一组是正确的，三角化成功的特征点数量最多
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    vector<float> vCosParallax; // 视差余弦值
    vCosParallax.reserve(vKeys1.size());

    // Camera 1 Projection Matrix K[I|0]
    cv::Mat P1(3,4,CV_32F,cv::Scalar(0));
    K.copyTo(P1.rowRange(0,3).colRange(0,3)); // P1 = K[I|0]

    cv::Mat O1 = cv::Mat::zeros(3,1,CV_32F);  // 第一帧的光心
