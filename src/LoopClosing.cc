...
{
    {
        ...
        mpCurrentKF->SetNotErase(); // 当前帧 不可以被别的线程删除
    ...
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase(); // 设置 可以被删除 标志
        return false;
    }
    ...
    /// 若之前几帧已经检测到闭环，则不需要经过词袋打分，直接计算sim3 进行几何一致性判断
    ...
    if(mnMergeNumCoincidences > 0)
    {
        ...
        g2o::Sim3 gScl(Converter::toMatrix3d(mTcl.rowRange(0, 3).colRange(0, 3)),Converter::toVector3d(mTcl.rowRange(0, 3).col(3)),1.0);
        g2o::Sim3 gScw = gScl * mg2oMergeSlw; // 通过
        int numProjMatches = 0;
        vector<MapPoint*> vpMatchedMPs;
...
/// 通过上一个检测到闭环的关键帧 计算当前关键帧的sim3（运动模型）
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    {
        //cout << endl << "-------------------------------" << endl;
        if(!pKFi || pKFi->isBad()) // 当前候选关键帧
            continue;

        /// 1.将 候选帧的共视帧 与 当前关键帧 进行词袋匹配
        ...
        // 当前候选关键帧 的匹配结果
        std::vector<std::vector<MapPoint*> > vvpMatchedMPs; // 所有共视帧的匹配点
        vvpMatchedMPs.resize(vpCovKFi.size());
        std::set<MapPoint*> spMatchedMPi;                   // 所有被匹配到的地图点集合
        int numBoWMatches = 0;                              // 所有地图点的数量总和

        KeyFrame* pMostBoWMatchesKF = pKFi; // 将候选帧及其共视帧 看作一个候选帧
        int nMostBoWNumMatches = 0; 

        // 当前关键帧 匹配到的地图点 与 地图点对应的关键帧
        ...
        /// 遍历 候选关键帧 的共视帧列表，进行特征匹配
        int nIndexMostBoWMatchesKF=0; // 匹配程度最高的共视帧（当前候选帧的共视帧中）
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            if(!vpCovKFi[j] || vpCovKFi[j]->isBad())
                continue;

            /// 当前帧 与 候选帧的共视帧 通过词袋进行特征匹配
            ...
            if (num > nMostBoWNumMatches)
            {
                ...
                nIndexMostBoWMatchesKF = j; // 匹配程度最高的共视帧（单个候选帧的共视帧列表中）
            }
        }

        bool bAbortByNearKF = false; // 该候选关键帧 在 当前关键帧 的附近
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            // 当前关键帧 与 候选关键帧 有相同的共视帧
            if(spConnectedKeyFrames.find(vpCovKFi[j]) != spConnectedKeyFrames.end())
            {
                bAbortByNearKF = true; // 该候选关键帧 在 当前关键帧 的附近
            ...
            for(int k=0; k < vvpMatchedMPs[j].size(); ++k)
            {
                ...
                {
                    spMatchedMPi.insert(pMPi_j); // 将地图点加入到集合中
                    numBoWMatches++;             // 所有地图点的数量总和
        ...
        {
            /// 2.当前关键帧 与 候选关键帧 计算Sim3（将 候选帧及其共视帧 看作一个候选帧）
            ...
            /// 构建 Sim3优化器（将 候选帧及其共视帧 看作一个候选帧 进行Sim3计算）
            ...
            if(bConverge)
            {
                /// 3.通过候选帧的共视帧，寻找更多的地图点来进行匹配，根据最终匹配上的地图点数，来决定是否闭环
                
                //cout <<"BoW: " << nInliers << " inliers in Sim3Solver" << endl;

                /// 3.1通过候选帧的共视帧，寻找更多的地图点
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            /// 6.从当前关键帧的共视帧中，检测是否有3个关键帧满足Sim3
                            
                            int nNumKFs = 0; // 满足Sim3的帧数
                            ...
                            {
                                nBestMatchesReproj = numProjOptMatches; // 最终匹配上的地图点数量
                                nBestNumCoindicendes = nNumKFs;         // 当前关键帧匹配上的共视帧数量
                                pBestMatchedKF = pMostBoWMatchesKF;     // 候选关键帧
                                g2oBestScw = gScw;
                                vpBestMapPoints = vpMapPoints;          // 候选帧及其共视帧 的所有地图点
                                vpBestMatchedMapPoints = vpMatchedMP;   // 候选帧及其共视帧 匹配上的地图点
                            }
                                         ...
                                         vector<MapPoint*> &vpMatchedMapPoints)
{
    ...
    set<KeyFrame*> spCheckKFs(vpCovKFm.begin(), vpCovKFm.end()); // 已经在集合里
    set<KeyFrame*> spCurrentCovisbles = pCurrentKF->GetConnectedKeyFrames(); // 当前关键帧的共视帧，用来剔除关键帧
    ...
    /// 寻找 候选帧及其一、二级共视帧 的地图点
    
    set<MapPoint*> spMapPoints; // 地图点集合 防止重复添加
    vpMapPoints.clear();        // 待匹配的地图点集合
...
void LoopClosing::CorrectLoop()
{
    ...
    // 使用当前帧的 sim3位姿, 来对 当前帧及其共视帧 的sim3位姿进行矫正
...

    {
        ...
        /// 1.使用当前帧的 sim3位姿, 来对 当前帧及其共视帧 的sim3位姿进行矫正
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)
            {
                ...
                CorrectedSim3[pKFi]=g2oCorrectedSiw; // 矫正后的 sim3位姿
            }
            ...
            NonCorrectedSim3[pKFi]=g2oSiw;           // 矫正前的 sim3位姿，s=1
        }
        ...
        /// 2.矫正 当前帧及其共视帧 所观测到的所有地图点 的3d坐标
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            KeyFrame* pKFi = mit->first;                            // 当前关键帧
            g2o::Sim3 g2oCorrectedSiw = mit->second;                // 矫正后的位姿
            g2o::Sim3 g2oCorrectedSwi = g2oCorrectedSiw.inverse();

            g2o::Sim3 g2oSiw =NonCorrectedSim3[pKFi];               // 矫正前的位姿

            vector<MapPoint*> vpMPsi = pKFi->GetMapPointMatches();  // 关键帧的地图点
            for(size_t iMP=0, endMPi = vpMPsi.size(); iMP<endMPi; iMP++)
            {
                ...
                if(pMPi->isBad())
                    continue;
                if(pMPi->mnCorrectedByKF==mpCurrentKF->mnId)        // 已经矫正过了
                    continue;
                ...
                pMPi->SetWorldPos(cvCorrectedP3Dw);                 // 矫正后的3d坐标
                pMPi->mnCorrectedByKF = mpCurrentKF->mnId;          // 已经被矫正过了
    ...
    /// 4.使用 闭环帧地图点 与 当前帧及共视帧 进行投影匹配，并融合新的观测关系
    ...
    map<KeyFrame*, set<KeyFrame*> > LoopConnections; // 闭环连接（只保留闭环边）
    ...
    for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
    {
        ...
        pKFi->UpdateConnections(); // 根据新的观测关系（闭环帧地图点 与 当前帧共视帧 的观测关系），会添加闭环连接
        LoopConnections[pKFi]=pKFi->GetConnectedKeyFrames();
        for(vector<KeyFrame*>::iterator vit_prev=vpPreviousNeighbors.begin(), vend_prev=vpPreviousNeighbors.end(); vit_prev!=vend_prev; vit_prev++)
        {
            LoopConnections[pKFi].erase(*vit_prev); // 旧连接，不是闭环边
        }
        for(vector<KeyFrame*>::iterator vit2=mvpCurrentConnectedKFs.begin(), vend2=mvpCurrentConnectedKFs.end(); vit2!=vend2; vit2++)
        {
            LoopConnections[pKFi].erase(*vit2);     // 与 共视帧 的连接
        }
    }
...
void LoopClosing::MergeLocal()
{
    ...
    ///------------------------1.获取两个Map的关键帧、地图点集合（local window）---------------------
    
    //Get the current KF and its neighbors(visual->covisibles; inertial->temporal+covisibles)
    set<KeyFrame*> spLocalWindowKFs; /// 当前地图的关键帧集合
    //Get MPs in the welding area from the current map
    set<MapPoint*> spLocalWindowMPs; /// 当前地图的地图点集合
    
    /// ----------------------------1.1 获取当前地图的关键帧集合 15个----------------------------------    
    ...
    /// ----------------------------1.3 获取Merge地图的关键帧集合 15个------------------------------------
    set<KeyFrame*> spMergeConnectedKFs; // Merge地图的关键帧集合
    if(pCurrentMap->IsInertial() && pMergeMap->IsInertial()) //TODO Check the correct initialization
    {
    ...
    vCorrectedSim3[mpCurrentKF]=g2oCorrectedScw;       // 矫正后的 sim3位姿
    vNonCorrectedSim3[mpCurrentKF]=g2oNonCorrectedScw; // 矫正前的 sim3位姿
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        if(pKFi!=mpCurrentKF)
        {
            ...
            g2o::Sim3 g2oSiw(Converter::toMatrix3d(Riw),Converter::toVector3d(tiw),1.0);
            //Pose without correction
            vNonCorrectedSim3[pKFi]=g2oSiw;       // 矫正前的，s=1
            ...
            vCorrectedSim3[pKFi]=g2oCorrectedSiw; // 矫正后的 sim3位姿
        }
        else
        {
        ...
        pKFi->mTcwMerge = correctedTiw; /// 矫正后的 SE3位姿
    ...
    ///--------------------------2.2 矫正当前地图的地图点 3d坐标（local window）-----------------------------
    ...
    for(MapPoint* pMPi : spLocalWindowMPs)
    {
        ...
        pMPi->mPosMerge = cvCorrectedP3Dw; /// 矫正后的 3d坐标
        //cout << "Rcor: " << Rcor << endl;
    ...
    /// ---------------------------4.Essential Graph（local window）-------------------------------
    ...
    // into the current keyframe and neighbors using corrected poses.
    // Fuse duplications.
    /// 5.1 使用Merge地图中的地图点 替换当前地图中的地图点
...
/// 使用 闭环帧地图点 与 当前帧及共视帧 进行投影匹配，并融合新的观测关系
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, vector<MapPoint*> &vpMapPoints)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        // 将 闭环帧地图点 与 当前关键帧 进行sim3投影匹配，并融合地图点
        ...
        for(int i=0; i<nLP;i++)
        {
            ...
            if(pRep)
            {
                ...
                num_replaces += 1;
                pRep->Replace(vpMapPoints[i]); // 融合 地图点
