...
/// BA优化：对 关键帧位姿 和 地图点坐标 进行优化
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<bool> vbNotIncludedMP; // 没有被观测到的地图点
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        vPoint->setId(id);
        vPoint->setMarginalized(true); // 边缘化，地图点的数量远多于关键帧
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(leftIndex != -1 && pKF->mvuRight[get<0>(mit->second)]<0) // 单目 或者 RGBD
            {
                const cv::KeyPoint &kpUn = pKF->mvKeysUn[leftIndex]; // 当前地图点 在 该关键帧 上的2d坐标

                Eigen::Matrix<double,2,1> obs; // 2d坐标
                obs << kpUn.pt.x, kpUn.pt.y;

                ORB_SLAM3::EdgeSE3ProjectXYZ* e = new ORB_SLAM3::EdgeSE3ProjectXYZ();

                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id))); // 地图点 3d坐标
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId))); // 关键帧位姿
                e->setMeasurement(obs); // 地图点 在 关键帧 上的2d坐标
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        {
            pKF->SetPose(Converter::toCvMat(SE3quat)); // 更新位姿
        }
        else
        {
            /*if(!vSE3->fixed())
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        if(nLoopKF==pMap->GetOriginKF()->mnId)
        {
            pMP->SetWorldPos(Converter::toCvMat(vPoint->estimate())); // 更新3d坐标
            pMP->UpdateNormalAndDepth(); // 更新平均观测方向以及可观测范围
        }
        else
        {
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    int nInitialCorrespondences=0; // 记录添加了多少边（地图点的数量）
    ...
    vnIndexEdgeStereo.reserve(N); // 每条边对应的特征点ID

    // delta用于构造核函数，以及判断是否外点的阈值（由卡方检验得到）
        ...
        if(pMP)
        {
            ...
            if(!pFrame->mpCamera2){
                ...
                if(pFrame->mvuRight[i]<0)
                {
                    ...
                    nInitialCorrespondences++; // 边的数量+1
                    ...
                    // 设置边，相连的顶点（顶点在边里的序号，位姿顶点）
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
                    e->setMeasurement(obs); // 观测为地图点的2D坐标
                    
                    // 信息矩阵：噪声协方差矩阵的逆（金字塔层数越高，噪声越大？）
                    const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);

                    // 添加鲁棒核函数，减少误匹配的影响（二范数：误匹配的影响特别大）
                    g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                    e->setRobustKernel(rk);
    ...
    if(nInitialCorrespondences<3)
        return 0; // 如果边的数量小于3,则跟踪失败

    // We perform 4 optimizations, after each optimization we classify observation as inlier/outlier
    // At the next optimization, outliers are not included, but at the end they can be classified as inliers again.
    /// 进行四次优化，每次优化后，重新区分内点和外点，外点不参与下一次的优化
    const float chi2Mono[4]={5.991,5.991,5.991,5.991};    // 是否为外点的阈值（huber核的delta）
    const float chi2Stereo[4]={7.815,7.815,7.815, 7.815};
    const int its[4]={10,10,10,10};                       // 每次优化的迭代次数

    int nBad=0; // 最后一次优化，外点的个数
    ...
    return nInitialCorrespondences-nBad; // 返回内点的数量
}

/// 局部BA优化：优化与 当前关键帧 有关联的 关键帧和地图点
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, vector<KeyFrame*> &vpNonEnoughOptKFs)
{
    // Local KeyFrames: First Breath Search from Current Keyframe
    /// Local KeyFrames: 与 当前关键帧 有共视的关键帧，作为g2o的顶点，参与优化
    ...
    /// Local MapPoints：Local KeyFrames能观测到的 所有地图点，作为g2o的顶点，参与优化
    ...
    /// Fixed Keyframes：能看到Local MapPoints 的其余关键帧，作为g2o的顶点，但不参与优化
    ...
    /// Set Local MapPoints vertices
    
    // Set MapPoint vertices
    // 将Local MapPoints里的 每个观测 都作为一条误差项边
    const int nExpectedSize = (lLocalKeyFrames.size()+lFixedCameras.size())*lLocalMapPoints.size();

    // 单目：保存g2o的 边、关键帧、地图点，用来删除外点所对应的观测关系
    vector<ORB_SLAM3::EdgeSE3ProjectXYZ*> vpEdgesMono; // 边
    vpEdgesMono.reserve(nExpectedSize);

    vector<ORB_SLAM3::EdgeSE3ProjectXYZToBody*> vpEdgesBody;
    vpEdgesBody.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFMono;                    // 关键帧
    vpEdgeKFMono.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFBody;
    vpEdgeKFBody.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeMono;              // 地图点
    vpMapPointEdgeMono.reserve(nExpectedSize);
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        //Set edges
        /// Set Local MapPoints edges
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            KeyFrame* pKFi = mit->first; // 观测到 该地图点 的关键帧
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(cam0Index != -1 && pKFi->mvuRight[cam0Index]<0) // 单目
                {
                    const cv::KeyPoint &kpUn = pKFi->mvKeysUn[cam0Index]; // 地图点 对应的 特征点
                    Eigen::Matrix<double,2,1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y; // 2d坐标，作为观测
    ...
    /// 将外点对应的 地图点与关键帧的观测关系 删除
    vector<pair<KeyFrame*,MapPoint*> > vToErase; // 外点
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        if(e->chi2()>5.991 || !e->isDepthPositive())
        {
            ...
            vToErase.push_back(make_pair(pKFi,pMP)); // 外点
        }
    }
...
{   
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        if(it!=CorrectedSim3.end()) // 属于 闭环边
        {
            vScw[nIDi] = it->second;
            VSim3->setEstimate(it->second); // 矫正后的 sim3
        }
        else // 不属于 闭环边
        {
            ...
            vScw[nIDi] = Siw;
            VSim3->setEstimate(Siw); // s=1
        }

        if(pKF->mnId==pMap->GetInitKFid()) // 闭环帧
            VSim3->setFixed(true);
    ...
    set<pair<long unsigned int,long unsigned int> > sInsertedEdges; // 已经添加的闭环边，用来防止重复添加
    ...
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        ...
        const set<KeyFrame*> &spConnections = mit->second; // 关键帧 的 闭环连接
        const g2o::Sim3 Siw = vScw[nIDi]; // 关键帧的 sim3
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            ...
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDj))); // 闭环帧
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDi))); // 关键帧
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        if(pParentKF)
        {
            ...
            g2o::EdgeSim3* e = new g2o::EdgeSim3();
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDj))); // 父关键帧
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDi))); // 子关键帧
            e->setMeasurement(Sji);
            count_kf++;
            count_spa_tree++;
            e->information() = matLambda;
        ...
        for(vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ...
            if(pKFn && pKFn!=pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
            {
                ...
                {
                    if(sInsertedEdges.count(make_pair(min(pKF->mnId,pKFn->mnId),max(pKF->mnId,pKFn->mnId)))) // 已经作为闭环边添加了
                        continue;
                    ...
                    en->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFn->mnId))); // 共视帧
                    en->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDi)));       // 当前帧
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        {
            nIDr = pMP->mnCorrectedReference; // 被矫正时，所在的关键帧
        }
        else
        {
            KeyFrame* pRefKF = pMP->GetReferenceKeyFrame();
            nIDr = pRefKF->mnId; // 地图点的参考关键帧
        }

        // 地图点所在关键帧的位姿
        g2o::Sim3 Srw = vScw[nIDr];                   // 矫正前
        g2o::Sim3 correctedSwr = vCorrectedSwc[nIDr]; // 矫正后

        cv::Mat P3Dw = pMP->GetWorldPos();
        Eigen::Matrix<double,3,1> eigP3Dw = Converter::toVector3d(P3Dw);
        Eigen::Matrix<double,3,1> eigCorrectedP3Dw = correctedSwr.map(Srw.map(eigP3Dw)); // 投影，再反投影 更新3d坐标
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    /// Set Sim3 vertex
    g2o::VertexSim3Expmap * vSim3 = new g2o::VertexSim3Expmap();    
    vSim3->_fix_scale=bFixScale; // 双目 或者 RGBD s=1
    ...
    for(int i=0; i<N; i++)
    {
        ...
        MapPoint* pMP1 = vpMapPoints1[i]; // 地图点1
        MapPoint* pMP2 = vpMatches1[i];   // 地图点2
        ...
        if(pMP1 && pMP2)
        {
            if(!pMP1->isBad() && !pMP2->isBad() && i2>=0)
            {
                ...
                cv::Mat P3D1w = pMP1->GetWorldPos(); // 世界坐标
                cv::Mat P3D1c = R1w*P3D1w + t1w; // 相机坐标
        ...
        obs1 << kpUn1.pt.x, kpUn1.pt.y; // 2d坐标

        g2o::EdgeSim3ProjectXYZ* e12 = new g2o::EdgeSim3ProjectXYZ();
        e12->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id2))); // 地图点2
        e12->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));   // sim3变换
        ...
        obs2 << kpUn2.pt.x, kpUn2.pt.y; // 2d坐标

        g2o::EdgeInverseSim3ProjectXYZ* e21 = new g2o::EdgeInverseSim3ProjectXYZ();

        e21->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id1))); // 地图点1
        e21->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));   // sim3变换
        ...
        /// 注意：两种边的类型不一样
        vpEdges12.push_back(e12); // 边1
        vpEdges21.push_back(e21); // 边2
    ...
    for(size_t i=0; i<vpEdges12.size();i++)
    {
        ...
        if(e12->chi2()>th2 || e21->chi2()>th2) // 外点
        {
    ...
    if(nCorrespondences-nBad<10) // 内点数小于10
        return 0;
    ...
    g2oS12= vSim3_recov->estimate(); // 修改sim3
