...
public:
    ...
    long unsigned int mnFuseTargetForKF; // 寻找相邻共视帧时，防止重复添加
    ...
    long unsigned int mnLoopQuery;  // 和闭环帧有共视，防止重复添加
    int mnLoopWords;                // 与闭环帧共视的单词数
    float mLoopScore;               // 与闭环帧的相似性打分
    
    long unsigned int mnRelocQuery; // 与关键帧关联的普通帧ID（用来判断是否与 当前普通帧 有关联）
    int mnRelocWords;               // 共视单词数
    float mRelocScore;
    ...
    cv::Mat mVwbBefGBA;
    IMU::Bias mBiasGBA;
    long unsigned int mnBAGlobalForKF; // 表示该关键帧的位姿，已经被矫正过了

    // Variables used by merging
    cv::Mat mTcwMerge;    // Merge之后的 SE3位姿
    cv::Mat mTcwBefMerge;
    cv::Mat mTwcBefMerge;
    cv::Mat mVwbMerge;
    cv::Mat mVwbBefMerge;
...
protected:
    ...
    std::map<KeyFrame*,int> mConnectedKeyFrameWeights;   // 共视边<共视帧，共视数>
    std::vector<KeyFrame*> mvpOrderedConnectedKeyFrames; // 排好序的共视帧（从大到小）
    std::vector<int> mvOrderedWeights;                   // 排好序的共视数（从大到小）
    // For save relation without pointer, this is necessary for save/load function
    std::map<long unsigned int, int> mBackupConnectedKeyFrameIdWeights;

    // Spanning Tree and Loop Edges
    bool mbFirstConnection;
    KeyFrame* mpParent;               // 父 关键帧
    std::set<KeyFrame*> mspChildrens; // 子 关键帧
    std::set<KeyFrame*> mspLoopEdges; //    闭环边
    ...
    // Bad flags
    bool mbNotErase;   // 不可以被删除，其它线程正在使用
    bool mbToBeErased; // 准备被删除，但正在被其他线程使用
    bool mbBad;        // 已经被删除
