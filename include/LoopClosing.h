...
public:
    ...
    // 一致性组 <候选组，连续数>，这里的候选组是候选帧共视产生的，用来表示一个地方
    typedef pair<set<KeyFrame*>,int> ConsistentGroup;
    typedef map<KeyFrame*,g2o::Sim3,std::less<KeyFrame*>, // <关键帧、sim3>
...
protected:
    ...
    /// 闭环检测 变量
    bool mbLoopDetected;           // “连续”三帧 检测到闭环
    int mnLoopNumCoincidences;     // “连续” 检测到闭环 的关键帧个数（不是绝对的“连续”，两闭环帧之间可能隔了一帧）
    int mnLoopNumNotFound;         // 连续 未检测到闭环 的关键帧个数（若连续两帧没检测到闭环，则重新开始检测闭环）
    KeyFrame* mpLoopLastCurrentKF; // 上一个 检测到闭环的关键帧
    g2o::Sim3 mg2oLoopSlw;         // 上一个 检测到闭环的关键帧 相对于世界的sim3位姿
    g2o::Sim3 mg2oLoopScw;         // 当前 检测到闭环的关键帧 相对于世界的sim3位姿
    KeyFrame* mpLoopMatchedKF;     // 闭环匹配帧
