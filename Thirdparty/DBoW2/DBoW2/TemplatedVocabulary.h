...
void TemplatedVocabulary<TDescriptor,F>::transform(
  ...
  BowVector &v, FeatureVector &fv, int levelsup) const  // 词袋向量 正向索引 索引层数4
{
  v.clear();
  fv.clear();
...
void TemplatedVocabulary<TDescriptor,F>::transform(const TDescriptor &feature,
  WordId &word_id, WordValue &weight, NodeId *nid, int levelsup) const // 叶子节点 IDF权重 正向索引 索引层数4
{ 
  ...
  // 正向索引的层数
  const int nid_level = m_L - levelsup;       // 6-4=2
  if(nid_level <= 0 && nid != NULL) *nid = 0; // root

  /// 逐层插入，直到叶子节点层
  NodeId final_id = 0;   // 当前层最近的节点
  int current_level = 0; // 当前层所在的层数

  do
  {
    ++current_level;                    // 当前的层数
    nodes = m_nodes[final_id].children; // 当前层的节点列表
    final_id = nodes[0];                // 当前层最近的节点
 
    // 计算特征到本层节点的最近距离 （非叶子节点的描述子，为所有子节点描述子的均值）
  ...
  word_id = m_nodes[final_id].word_id; // 叶子节点
  weight = m_nodes[final_id].weight;   // IDF权重
}
...
bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile(const std::string &filename)
{
    ...
    // 根据第一行信息，获取字典的 分类数k、树深度d、L1范数、TF-IDF
    string s;
    getline(f,s);
    stringstream ss;
    ss << s;
    ss >> m_k; // 分类数量 k = 10
    ss >> m_L; // 树的深度  d = 6   单词的数量，为叶子节点的个数10^6
    int n1, n2;
    ss >> n1;
    ss >> n2;
    ...
    m_scoring = (ScoringType)n1;     // 评分类型为0：L1范数
    m_weighting = (WeightingType)n2; // 权重类型为0：TF-IDF
    createScoringObject();
